...
export const authApi = {
  ...
  update: (data: UpdateUserRequest | FormData) =>

    post<ApiResponse<User>>('/v1/auth/update', data, false),
...
export const quizzesApi = {
  ...
  create: (data: CreateQuizRequest, onProgress?: (progress: number) => void) => {
    const formData = buildFormData(data as unknown as Record<string, unknown>);
    return postMultipart<ApiResponse<Quiz>>('/v1/quiz', formData, onProgress);
  },

  createWithoutFiles: (data: CreateQuizRequest) => {
    return post<ApiResponse<Quiz>>('/v1/quiz', data, false);
  },



  update: (id: number, data: Partial<CreateQuizRequest>, onProgress?: (progress: number) => void) => {
    const formData = buildFormData(data as unknown as Record<string, unknown>);
    return putMultipart<ApiResponse<Quiz>>(`/v1/quiz/${id}`, formData, onProgress);
  },
...
export const instructorsApi = {
  ...
  update: (id: string, data: Partial<CreateStudentRequest>, onProgress?: (progress: number) => void) => {
    ...
    // Extract can_use_activations to handle as JSON boolean (FormData converts booleans to strings)

    const { can_use_activations, ...restData } = data;

    const formData = buildFormData(restData as unknown as Record<string, unknown>);

    return putMultipart<ApiResponse<Student>>(`/v1/student/${id}`, formData, onProgress).then((result) => {

      if (can_use_activations !== undefined) {

        return put<ApiResponse<Student>>(`/v1/student/${id}`, { can_use_activations }).then(() => result);
  ...
  // JSON-based update to preserve boolean types (FormData converts to strings)

  updateCanUseActivations: (id: string, can_use_activations: boolean) =>

    put<ApiResponse<Student>>(`/v1/student/${id}`, { can_use_activations }),
