...
export default function EditCodePage() {
  ...
  if (isLoadingCode || isLoadingItems) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] gap-4">
        ...
        <p className="text-[#64748B]">{isLoadingCode ? 'Loading code data...' : 'Loading items...'}</p>
      </div>
    );
  }
  ...
  if (!code) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] gap-4">
        <p className="text-red-600">Code not found</p>
        <Link
          href="/activation"
  ...
  return (
    <div className="flex flex-col gap-8 max-w-4xl mx-auto pb-12">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-[#1E293B]">Edit Activation Code</h1>
          <p className="text-sm text-[#64748B] mt-0.5">Update code details and assignment.</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-6">
        ...
        <section className="bg-white rounded-2xl border border-[#F1F5F9] shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-[#F1F5F9] bg-[#F8FAFC]/50">
            <h2 className="text-sm font-bold text-[#1E293B] uppercase tracking-wider">Code Details</h2>
          </div>
          <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* Code Value */}
            <div className="flex flex-col gap-2">
              <label className="text-[13px] font-bold text-[#475569]">Code <span className="text-red-500">*</span></label>
              <input
                type="text"
                ...
                required
              />
              <p className="text-xs text-[#94A3B8]">Unique activation code</p>
            </div>
            ...
            <div className="flex flex-col gap-2 md:col-span-2">
              ...
              {itemId && (
                <div className="px-3 py-2 bg-[#EEF2FF] border border-[#2137D6]/20 rounded-lg">
                  <span className="text-sm font-medium text-[#2137D6]">Current: {getCurrentItemName()}</span>
                </div>
              )}
              <select
                className="w-full px-4 py-2.5 bg-[#F8FAFC] border border-[#E2E8F0] rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#2137D6] focus:ring-opacity-10 transition-all appearance-none cursor-pointer"
