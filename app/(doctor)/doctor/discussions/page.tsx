...
export default function DoctorDiscussionsPage() {
  ...
  const DiscussionNode = ({ discussion, isRoot = false }: { discussion: any; isRoot?: boolean }) => {
          ...
          {hasReplies && (
            <div className="mt-2 border-l border-[#E2E8F0] ml-4 sm:ml-5">
              {discussion.replies.map((reply: any) => (
                <DiscussionNode key={reply.id} discussion={reply} />
              ))}
            </div>
          )}
        </div>
      ...
      <div className="space-y-6">
        {discussionsTree.length === 0 ? (
          ...
          discussionsTree.map((discussion: any) => (
            <DiscussionNode key={discussion.id} discussion={discussion} isRoot={true} />
          ))
        )}
      </div>
    </div>
